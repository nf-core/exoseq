...
First, go to the [nfcore/ExoSeq releases page](https://github.com/nf-core/ExoSeq/releases) and find the latest version number - numeric only (eg. `0.8`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 0.8`.
...
You can find the keys to specify the genomes in the [iGenomes config file](https://github.com/nf-core/ExoSeq/blob/master/conf/igenomes.config). Common genomes that are supported are:
