...
```bash
nextflow pull SciLifeLab/nfcore/ExoSeq
```

### Reproducibility
It's a good idea to specify a pipeline version when running the pipeline on your data. This ensures that a specific version of the pipeline code and software are used when you run your pipeline. If you keep using the same tag, you'll be running the same version of the pipeline, even if there have been changes to the code since.

First, go to the [nfcore/ExoSeq releases page](https://github.com/SciLifeLab/nfcore/ExoSeq/releases) and find the latest version number - numeric only (eg. `0.8`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 0.8`.
...
You can find the keys to specify the genomes in the [iGenomes config file](https://github.com/SciLifeLab/nfcore/ExoSeq/blob/master/conf/igenomes.config). Common genomes that are supported are:
