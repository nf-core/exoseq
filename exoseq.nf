...
/*
Package name: NGI-EXOseq
Package syntax: Nextflow (Groovy)
Package framework: Developed based on GATK's best practise, takes set of FASTQ files
and performs alignment, recalibration, realignment, variant calling and vairant evaluation
Package author: Senthilkumar Panneerselvam <senthilkumar.panneerselvam@scilifelab.se>
Package homepage: https://github.com/SciLifeLab/NGI-EXOseq
*/
...
Usage: nextflow exoseq.nf --reads 'P1234*_R{1,2}.fastq.gz' --genome GRCh37
...
if (!params.metaFiles[ params.genome ] && ['gfasta', 'bwa_index', 'dbsnp', 'hapmap', 'omni'].count{ params[it] } != 5){
        exit 1, "Genome '${params.genome}' is not available in pre-defined config, so provide all genome specific "+
...
process bwaAlign {
    ...
    script:
        """
        bwa mem -t 8 -k 2 $params.bwa_index $fastq > ${sample}_bwa.sam
        """
}
...
process fastqToSam {
    ...
    script:
        ...
        """
        java -jar \$PICARD_HOME/picard.jar FastqToSam FASTQ=${fastq[0]} FASTQ2=${fastq[1]} \\
            QUALITY_FORMAT=Standard OUTPUT=${sample}_unaligned.bam READ_GROUP_NAME=2 \\
            SAMPLE_NAME=$sam_name PLATFORM_UNIT=$fc_lane VALIDATION_STRINGENCY=SILENT \\
            PLATFORM=illumina TMP_DIR=tmp SORT_ORDER=queryname MIN_Q=0 MAX_Q=93 QUIET=false \\
            COMPRESSION_LEVEL=5 MAX_RECORDS_IN_RAM=500000 VERBOSITY=INFO \\
            CREATE_INDEX=false CREATE_MD5_FILE=false GA4GH_CLIENT_SECRETS=''
        """
}
...
process mergeLaneBam {
    ...
    script:
        """
        java -jar \$PICARD_HOME/picard.jar MergeBamAlignment UNMAPPED_BAM=$unaln_bam ALIGNED_BAM=$aln_sam \\
            OUTPUT=${sample}_merged.bam REFERENCE_SEQUENCE=$params.gfasta PAIRED_RUN=true TMP_DIR=tmp \\
            CLIP_ADAPTERS=true IS_BISULFITE_SEQUENCE=false ALIGNED_READS_ONLY=false SORT_ORDER=coordinate \\
            READ1_TRIM=0 READ2_TRIM=0 MAX_INSERTIONS_OR_DELETIONS=1 CLIP_OVERLAPPING_READS=true VERBOSITY=INFO \\
            QUIET=false VALIDATION_STRINGENCY=SILENT COMPRESSION_LEVEL=5 MAX_RECORDS_IN_RAM=500000 \\
            CREATE_INDEX=false CREATE_MD5_FILE=false GA4GH_CLIENT_SECRETS=''
        """
}
...
process sortLanesBam {
    ...
    script:
        """
        java -jar \$PICARD_HOME/picard.jar SortSam INPUT=$lane_bam OUTPUT=${sample}_sorted.bam VERBOSITY=INFO \\
            SORT_ORDER=coordinate TMP_DIR=tmp VALIDATION_STRINGENCY=SILENT MAX_RECORDS_IN_RAM=500000 \\
            QUIET=false COMPRESSION_LEVEL=5 CREATE_INDEX=false CREATE_MD5_FILE=false GA4GH_CLIENT_SECRETS=''
        """
}
...
process mergeSampleBam {
    ...
    script:
        if (sample_bam.properties.target)
            """
            java -jar \$PICARD_HOME/picard.jar MergeSamFiles ${sample_bam.target.flatten{"INPUT=$it"}.join(' ')} \\
            OUTPUT=${sample}_sorted.bam SORT_ORDER=coordinate TMP_DIR=tmp VALIDATION_STRINGENCY=SILENT VERBOSITY=INFO \\
            ASSUME_SORTED=false MERGE_SEQUENCE_DICTIONARIES=false USE_THREADING=false QUIET=false COMPRESSION_LEVEL=5 \\
...
process markDuplicate {
    ...
    script:
        """
        java -jar \$PICARD_HOME/picard.jar MarkDuplicates INPUT=$sorted_bam OUTPUT=${sample}_markdup.bam \\
            METRICS_FILE=${sample}.dup_metrics TMP_DIR=tmp VALIDATION_STRINGENCY=SILENT REMOVE_DUPLICATES=false \\
            ASSUME_SORTED=false MAX_SEQUENCES_FOR_DISK_READ_ENDS_MAP=50000 MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=8000 \\
            SORTING_COLLECTION_SIZE_RATIO=0.25 READ_NAME_REGEX=\"[a-zA-Z0-9]+:[0-9]:([0-9]+):([0-9]+):([0-9]+).*\" \\
            OPTICAL_DUPLICATE_PIXEL_DISTANCE=100 VERBOSITY=INFO QUIET=false COMPRESSION_LEVEL=5 CREATE_INDEX=false \\
            MAX_RECORDS_IN_RAM=500000  CREATE_MD5_FILE=false GA4GH_CLIENT_SECRETS=''
        """
}
...
process recalibrate {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T BaseRecalibrator -I $markdup_bam -R $params.gfasta \\
            -o ${sample}_table.recal -cov ReadGroupCovariate -cov QualityScoreCovariate -cov CycleCovariate \\
            -cov ContextCovariate -U -OQ --default_platform illumina --knownSites $params.dbsnp -l INFO

        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T PrintReads -BQSR ${sample}_table.recal -I $markdup_bam \\
            -R $params.gfasta -o ${sample}_recal.bam -baq RECALCULATE -U -OQ -l INFO
        """
}
...
process realign {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T RealignerTargetCreator -I $recal_bam \\
            -R $params.gfasta -o ${sample}_realign.intervals --known $params.dbsnp -l INFO
        
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T IndelRealigner -I $recal_bam -R $params.gfasta \\
            -targetIntervals ${sample}_realign.intervals -o ${sample}_realign.bam -l INFO
        """
}
...
process calculateMetrics {
    ...
    script:
        """
        java -jar \$PICARD_HOME/picard.jar CollectAlignmentSummaryMetrics INPUT=$aligned_bam OUTPUT=${sample}.align_metrics \\
            REFERENCE_SEQUENCE=$params.gfasta VALIDATION_STRINGENCY=SILENT MAX_INSERT_SIZE=100000 ASSUME_SORTED=true \\
            IS_BISULFITE_SEQUENCED=false METRIC_ACCUMULATION_LEVEL="ALL_READS" STOP_AFTER=0 VERBOSITY=INFO QUIET=false \\
            COMPRESSION_LEVEL=5 MAX_RECORDS_IN_RAM=500000 CREATE_INDEX=false CREATE_MD5_FILE=false GA4GH_CLIENT_SECRETS=''
        
        java -jar \$PICARD_HOME/picard.jar CollectInsertSizeMetrics HISTOGRAM_FILE=${sample}_insert.pdf INPUT=$aligned_bam \\
            OUTPUT=${sample}.insert_metrics VALIDATION_STRINGENCY=SILENT DEVIATIONS=10.0 MINIMUM_PCT=0.05 STOP_AFTER=0 \\
            METRIC_ACCUMULATION_LEVEL="ALL_READS" ASSUME_SORTED=true VERBOSITY=INFO QUIET=false COMPRESSION_LEVEL=5 \\
            MAX_RECORDS_IN_RAM=500000 CREATE_INDEX=false CREATE_MD5_FILE=false GA4GH_CLIENT_SECRETS=''
        
        java -jar \$PICARD_HOME/picard.jar CalculateHsMetrics BAIT_INTERVALS=$params.bait TARGET_INTERVALS=$params.target \\
            INPUT=$aligned_bam OUTPUT=${sample}.hs_metrics METRIC_ACCUMULATION_LEVEL="ALL_READS" VERBOSITY=INFO \\
            VALIDATION_STRINGENCY=SILENT QUIET=false COMPRESSION_LEVEL=5 MAX_RECORDS_IN_RAM=500000 CREATE_INDEX=false \\
            CREATE_MD5_FILE=false GA4GH_CLIENT_SECRETS=''
        """
}
...
process variantCall {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T HaplotypeCaller -I $realign_bam -R $params.gfasta \\
            -o ${sample}_variants.vcf --annotation HaplotypeScore --annotation MappingQualityRankSumTest \\
            --annotation QualByDepth --annotation ReadPosRankSumTest --annotation RMSMappingQuality \\
            --annotation FisherStrand --annotation Coverage --standard_min_confidence_threshold_for_calling 30.0 \\
...
process variantSelect {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T SelectVariants -R $params.gfasta \\
            --variant $raw_vcf --out ${sample}_snp.vcf --selectTypeToInclude SNP
        
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T SelectVariants -R $params.gfasta --variant $raw_vcf \\
            --out ${sample}_indels.vcf --selectTypeToInclude INDEL --selectTypeToInclude MIXED --selectTypeToInclude MNP \\
            --selectTypeToInclude SYMBOLIC --selectTypeToInclude NO_VARIATION
        """
}
...
process filterSnp {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T VariantRecalibrator -R $params.gfasta --input $raw_snp \\
            --maxGaussians 4 --recal_file ${sample}_snp.recal --tranches_file ${sample}_snp.tranches \\
            ...
            --mode SNP -an QD -an FS -an MQ

        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T ApplyRecalibration -R $params.gfasta \\
            --out ${sample}_filtered_snp.vcf --input $raw_snp --mode SNP \\
            --tranches_file ${sample}_snp.tranches --recal_file ${sample}_snp.recal
        """
}
...
process filterIndel {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T VariantFiltration -R $params.gfasta --variant $raw_indel \\
            --out ${sample}_filtered_indels.vcf --filterName GATKStandardQD --filterExpression "QD < 2.0" \\
            --filterName GATKStandardReadPosRankSum --filterExpression "ReadPosRankSum < -20.0" \\
            --filterName GATKStandardFS --filterExpression "FS > 200.0"
        """
}
...
process combineVariants {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T CombineVariants -R $params.gfasta \\
            --out ${sample}_combined_variants.vcf --genotypemergeoption PRIORITIZE \\
            --variant:${sample}_SNP_filtered $fsnp --variant:${sample}_indels_filtered $findel \\
...
process haplotypePhasing {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T ReadBackedPhasing -R $params.gfasta -I $bam \\
            --variant $vcf --out ${sample}_combined_phased_variants.vcf
        """
}
...
process variantEvaluate {
    ...
    script:
        """
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T VariantEval -R $params.gfasta --eval $phased_vcf \\
            --dbsnp $params.dbsnp -o ${sample}_combined_phased_variants.eval -L $params.target \\
            --doNotUseAllStandardModules --evalModule TiTvVariantEvaluator --evalModule CountVariants \\
            --evalModule CompOverlap --evalModule ValidationReport --stratificationModule Filter -l INFO 
        """
}
...
process variantAnnotate {
    ...
    script:
        """
        java -Xmx6g -jar \$SNPEFF_HOME/snpEff.jar -c \$SNPEFF_HOME/snpEff.config -i vcf -o gatk -o vcf \\
            -filterInterval $params.target_bed GRCh37.75 $phased_vcf > ${sample}_combined_phased_variants.snpeff
        
        java -jar \$GATK_HOME/GenomeAnalysisTK.jar -T VariantAnnotator -R $params.gfasta -A SnpEff --variant $phased_vcf \\
            --snpEffFile ${sample}_combined_phased_variants.snpeff --out ${sample}_combined_phased_annotated_variants.vcf
        """
}
